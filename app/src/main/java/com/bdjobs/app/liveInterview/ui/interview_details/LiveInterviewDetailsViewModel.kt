...
class LiveInterviewDetailsViewModel(
    ...
    private val calendarInfos = MutableLiveData<ArrayList<String>>().apply {
        value = arrayListOf()
    }
    ...
    private val _liveInterviewDetailsData =
        MutableLiveData<MutableList<LiveInterviewDetails.Data?>>()
    val liveInterviewDetailsData: LiveData<MutableList<LiveInterviewDetails.Data?>> =
        _liveInterviewDetailsData
    ...
    fun getLiveInterviewDetails() {
        ...
        viewModelScope.launch {
            try {
                ...
                val listSize = _liveInterviewDetailsData.value?.size

                if (_liveInterviewDetailsData.value != null && _liveInterviewDetailsData.value!!.isNotEmpty()) {
                    response.data?.let {
                        _liveInterviewDetailsData.value?.get(listSize!!.minus(1))?.showUndo =
                            _commonData.value?.showUndo
                    }

                    //for green section
                    showConfirmationSection.value =
                        liveInterviewDetailsData.value?.get(listSize!!.minus(1))?.confimationStatus == "0" || liveInterviewDetailsData.value?.get(
                            listSize!!.minus(1)
                        )?.confimationStatus == "5"

                    //exam date and time
                    examDate.value = liveInterviewDetailsData.value?.get(listSize!!.minus(1))?.examDate
                    examTime.value = liveInterviewDetailsData.value?.get(listSize!!.minus(1))?.examTime
                    interviewDateTime = "${liveInterviewDetailsData.value?.get(listSize!!.minus(1))?.examDate} ${
                        liveInterviewDetailsData.value?.get(listSize!!.minus(1))?.examTime
                    }"
                    _processID.value = liveInterviewDetailsData.value?.get(listSize!!.minus(1))?.processId
                    _levelStatus.value = liveInterviewDetailsData.value?.get(listSize!!.minus(1))?.levelstatus

                    applyId = commonData.value?.applyId.toString()
                    _applyID.value = commonData.value?.applyId.toString()
                    invitationId = liveInterviewDetailsData.value?.get(listSize!!.minus(1))?.invitationId.toString()
                    ...
                    val confirmationStatus =
                        liveInterviewDetailsData.value?.get(listSize!!.minus(1))?.confimationStatus
                    val confirmationDate = liveInterviewDetailsData.value?.get(listSize!!.minus(1))?.confirmationDate
    ...
    private fun setTimer(interviewDateTime: String) {
        ...
        endCalendar.time = simpleDateFormat.parse(interviewDateTime)!!
        ...
        timer = object : CountDownTimer(totalMillis, 1000) {
            ...
            override fun onTick(millisUntilFinished: Long) {
                ...
                var muf = millisUntilFinished
                val days: Long = TimeUnit.MILLISECONDS.toDays(muf)
                muf -= TimeUnit.DAYS.toMillis(days)
                val hours: Long = TimeUnit.MILLISECONDS.toHours(muf)
                muf -= TimeUnit.HOURS.toMillis(hours)
                val minutes: Long = TimeUnit.MILLISECONDS.toMinutes(muf)
                muf -= TimeUnit.MINUTES.toMillis(minutes)
                val seconds: Long = TimeUnit.MILLISECONDS.toSeconds(muf)
        ...
        return withContext(Dispatchers.IO) {
            ...
            val values = ContentValues().apply {
                ...
                put(
                    ...
                    "Live Interview from ${data?.companyName} for the position ${data?.jobTitle}"
                )
