...
import com.bdjobs.app.editResume.adapters.ArmyEmpHistoryAdapter
...
class ArmyEmpHisViewFragment : Fragment() {
    ...
    private var adapter: ArmyEmpHistoryAdapter? = null
    private var arr: ArrayList<ArmydataItem>? = null
            ...
            override fun onResponse(call: Call<List<GetArmyEmpHis>>, response: Response<List<GetArmyEmpHis>>) {
                try {
                    if (response.isSuccessful) {
                        ...
                        val dModel = respo?.armydata?.get(0)
                        if (dModel != null) {
